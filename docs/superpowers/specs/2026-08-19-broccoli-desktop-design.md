...
- On network failure, the client retains up to ten minutes of continuously captured frames (five minutes of wall clock with both channels streaming, ~29 MB at worst — transcript completeness is deliberately favored over resources) in a single bounded per-run queue, retries with backoff, and automatically reopens the same session if the original active window expires. If the bound is exceeded, or the queue fills because the socket has fallen behind, the oldest frame is dropped and the user is told; capture is never blocked waiting on the network.
...
- AudioCapture enumerates devices through PyAudioWPatch, listing only WASAPI endpoints (the MME/DirectSound duplicates route capture through emulation layers); enumeration is cached for a short TTL and runs off the event loop so it cannot freeze the local API. Each source opens at its endpoint's native mix format, multichannel blocks are averaged to mono off the callback thread, transient xrun flags are tolerated and logged rather than treated as device loss, and a per-source watchdog polling stream health is what turns an actually dead endpoint into device-selection recovery.
- AudioPipeline accepts 20 ms mono PCM blocks continuously at whatever rate the device natively runs — there is no client-side VAD gate — resamples them to mono PCM16 at 24 kHz through one stateful soxr stream per channel (per-block one-shot conversion restarted the filter at every boundary and audibly degraded the audio the transcription model receives), then batches 100 ms protocol frames. A mute discards the channel's stream state with its partial frame. The backend applies VAD server-side.
